  ...
  if (!interview) {
    return (
      <div className="animate-fade-in-up text-center py-20">
        ...
        <p className="text-slate-400 mb-6">This interview may have been deleted.</p>
        <button
          onClick={() => router.push("/interviews")}
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/[0.06] text-white text-sm font-medium hover:bg-white/[0.1] transition-all"
        >
          ← Back to Interviews
        </button>
      </div>
        ...
        <div className="p-6 sm:p-8 border-b border-white/[0.06]">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div className="flex items-start gap-4">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-500/20 to-violet-500/20 border border-indigo-500/20 flex items-center justify-center text-indigo-400 font-bold text-xl flex-shrink-0">
                {interview.companyName.charAt(0).toUpperCase()}
              </div>
              <div>
                <h1 className="text-2xl font-bold text-white">{interview.companyName}</h1>
                <p className="text-slate-400 mt-0.5">{interview.position}</p>
                <div className="mt-3">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium text-slate-300 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] transition-all"
              >
            ...
            <div>
              ...
              <div className="space-y-2">
                {interview.questions.map((q, i) => (
                  <div
                    key={i}
                    className="flex gap-3 p-3 rounded-xl bg-white/[0.02] border border-white/[0.04] animate-slide-in"
                    style={{ animationDelay: `${i * 50}ms` }}
                  >
                    <span className="text-xs text-indigo-400 font-mono mt-0.5 w-5 text-right flex-shrink-0">
                      {i + 1}.
                    </span>
