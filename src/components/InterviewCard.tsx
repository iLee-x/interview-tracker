...
export default function InterviewCard({ interview, onDelete }: InterviewCardProps) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.04] transition-all duration-300 hover:border-white/[0.1] hover:shadow-xl hover:shadow-indigo-500/[0.03]">
      <div className="p-5">
        {/* Header */}
        ...
        <div className="flex flex-wrap items-center gap-3 text-sm text-slate-400 mb-3">
          <span className="flex items-center gap-1.5">
            ...
            <span className={isPast ? "text-slate-500" : "text-indigo-400"}>{formattedDate}</span>
          </span>
          <span className="flex items-center gap-1.5">
            <svg className="w-4 h-4 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        {interview.questions.length > 0 && (
          <div className="mb-3 space-y-1">
            {interview.questions.slice(0, 2).map((q, i) => (
              <p key={i} className="text-xs text-slate-500 truncate pl-3 border-l border-slate-700">
                {q}
              </p>
            ))}
        ...
        <div className="flex items-center gap-2 pt-3 border-t border-white/[0.04]">
          <Link
            ...
            className="flex-1 text-center text-sm font-medium text-slate-400 hover:text-white py-2 rounded-lg hover:bg-white/[0.04] transition-all"
          >
            View Details
          </Link>
          {onDelete && (
